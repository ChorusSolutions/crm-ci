...
namespace Dynamics.Consent.Click.Demo
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            try
            {
                ...
                ServiceStore<IOrganizationService>.Singleton.Store(_orgService);

                var container = new UnityContainer();
                Services.Dependencies.Setup.Initialize(container);

                Guid sentEmail = new Guid("{040364CA-7E42-4B5D-B3F8-66826CDCF956}");

                var ServiceMethod = container.Resolve<ClickDimensionsEmailRetriever>();
               Console.WriteLine(ServiceMethod.GetTemplateId(sentEmail));
